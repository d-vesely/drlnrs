...
from ..algorithms.qr import get_trainee, act, get_next_action, get_tau
from ..algorithms.dist_rl_utils import get_huber_loss
from .trainer_base import _TrainerBase


class TrainerQR(_TrainerBase):
    ...
    def set_trainee(self):
        ...
        self._print_num_params(self.dqn.parameters(), name="DQN_QR")
    ...
    def _training_step(self, batch, step_i, gamma, print_q):
        ...
        with torch.no_grad():
            ...
            next_quantiles = self.target_dqn(next_state_rep, candidates)
        ...
        loss = huber_loss * torch.abs(get_tau(self.n_quantiles, self.device) -
                                      (td_error < 0).float())
